...
document.addEventListener("DOMContentLoaded", () => {
	...
	/* 회원 가입 약관 전체 동의 안하면 가입 불가 */
	document.addEventListener("DOMContentLoaded", () => {
	    const joinForm = document.querySelector("#joinModal form"); // 회원가입 폼
	    const agreeTerms = document.getElementById("agreeTerms"); // 이용약관 동의 체크박스
	    const agreePrivacy = document.getElementById("agreePrivacy"); // 개인정보 동의 체크박스

	    joinForm.addEventListener("submit", (event) => {
	        if (!agreeTerms.checked || !agreePrivacy.checked) {
	            alert("이용약관 및 개인정보 처리방침에 동의 해야 가입이 가능합니다.");
